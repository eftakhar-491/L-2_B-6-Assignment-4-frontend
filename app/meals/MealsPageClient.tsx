...
export interface MealsQueryState {
  searchTerm: string;
  categoryId: string;
  dietary: string;
  minPrice: string;
...
const normalizeQueryState = (
  query: Partial<MealsQueryState>,
): MealsQueryState => ({
  searchTerm: query.searchTerm?.trim() ?? "",
  categoryId: query.categoryId?.trim() ?? "",
  dietary: toDietarySlug(query.dietary ?? ""),
  minPrice: query.minPrice?.trim() ?? "",
  maxPrice: query.maxPrice?.trim() ?? "",
});

const buildMealsHref = (params: MealsQueryState) => {
...
export default function MealsPageClient({
  ...
  initialQuery: MealsQueryState;
}) {
  const router = useRouter();
  const [query, setQuery] = useState<MealsQueryState>(() =>
    normalizeQueryState(initialQuery),
  );

  const searchTerm = query.searchTerm;
  const selectedCategoryId = query.categoryId;
  const selectedDietarySlug = query.dietary;
  const minPriceInput = query.minPrice;
  const maxPriceInput = query.maxPrice;
  ...
  useEffect(() => {
    setQuery(normalizeQueryState(initialQuery));
  }, [
    initialQuery.searchTerm,
    initialQuery.categoryId,
    initialQuery.dietary,
  ...
  const pushFilters = useCallback(
    (params: Partial<MealsQueryState>) => {
      const next = normalizeQueryState({
        searchTerm,
      ...
      setQuery(next);
      router.replace(buildMealsHref(next), { scroll: false });
    },
    [
      router,
      searchTerm,
